...
{
    // ── Seed data (replace with DB queries later) ─────────────────────────────
    ...
    {
        if ($item['quantity'] <= $item['reorder']) return 'Low';
        if ($item['quantity'] / $item['max'] <= 0.65) return 'Medium';
        return 'Full';
    }
    ...
    public function stockInStore(Request $request)
    {
        ...
        $transactions[] = [
            ...
            'type' => 'in', 'item_id' => $items[$index]['id'], 'item_name' => $items[$index]['name'],
            'quantity' => (int) $request->quantity, 'date' => $request->date,
            'supplier' => $request->supplier, 'reason' => null, 'service_ref' => null,
        ];
    ...
    public function stockOutStore(Request $request)
    {
        ...
        $transactions[] = [
            'id' => 'T' . str_pad((string) (count($transactions) + 1), 3, '0', STR_PAD_LEFT),
            'type' => 'out', 'item_id' => $items[$index]['id'], 'item_name' => $items[$index]['name'],
            'quantity' => (int) $request->quantity, 'date' => now()->toDateString(),
            'supplier' => null, 'reason' => $request->reason, 'service_ref' => $request->service_ref,
        ];
        session(['staff_items' => $items, 'staff_transactions' => $transactions]);
        session()->forget('staff_pending_out');
        return redirect()->route('staff.view-inventory')->with('toast', 'Stock-Out recorded successfully.');
